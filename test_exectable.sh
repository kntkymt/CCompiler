...
assert 3 "int sum(a,b) { return a + b; } int main() { return sum(1, 2); }"
assert 12 "int sum(a,b) { return a * 10 + b; } int main() { return sum(1, 2); }"
assert 74 "int sum(a,b,c) { d = 10; return a * 2 + b * 3 + c * 4 + d * 5; } int main() { a = 1; return sum(a * 3, 2, 3); }"
